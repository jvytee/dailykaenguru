...
use telegram_bot::{prelude::*, Api, Error, Message, MessageChat, MessageKind, UpdateKind, InputFileUpload};
use tokio::time;
use tokio_stream::StreamExt;

type ChatCache = Arc<Mutex<HashSet<MessageChat>>>;

async fn deliver_comic(api: &Api, chat_cache: ChatCache, delivery_time: NaiveTime, config: &DownloadConfig) {
    loop {
	...
	for chat in chats.iter() {
	    let _ = api.send(chat.photo(&comic))
		.await
